...
public class VkBridgeController : MonoBehaviour
{
    ...
    [DllImport("__Internal")] public static extern void _VKWebAppShowBannerAd(string banner_location, string layout_type, bool can_close);
    ...
    public void VKWebAppShowBannerAd(VKWebAppShowBannerAdStruct paramsAd, UnityAction<VKWebAppShowNativeAdsResultStruct> actionResult)
    {
#if !UNITY_EDITOR
_actionResultAdsShow = actionResult;
if (paramsAd.can_close != null)
{
  _VKWebAppShowBannerAd(
    Enum.GetName(typeof(BannerAdFormat), paramsAd.banner_location),
    Enum.GetName(typeof(BannerLayoutType), paramsAd.layout_type),
    bool.Parse(paramsAd.can_close.ToString())
);
}
else
{
    _VKWebAppShowNativeAds(Enum.GetName(typeof(BannerAdFormat), paramsAd.layout_type), false);
