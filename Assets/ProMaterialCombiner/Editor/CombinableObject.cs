...
namespace ProMaterialCombiner {
    ...
    public class CombinableObject {
        ...
        public void CombineObject(string customAtlasName, bool generatePrefab) {
            if(objectToCombine == null || !isObjectCorrectlyAssembled) {
                Debug.LogError("ERROR: Cant combine object!");
                return;
            }
            materialCombiner.CombineMaterials(usesSkinnedMeshRenderer, customAtlasName, generatePrefab);
        }

        public int GetAtlasSize() {
            return materialCombiner.GetAproxAtlasSize(materialCombiner.ObjectMaterials[0]);
        }

        //this is already set in an Area called from OnGUI() on ProMaterialCombinerMenu.cs
        public void DrawGUI() {//refers to the area we are going to draw this GUI into
            if(!IsCorrectlyAssembled)
                return;
            GUILayout.BeginVertical();
            int atlasSize = materialCombiner.GetAproxAtlasSize(materialCombiner.ObjectMaterials[0]);
